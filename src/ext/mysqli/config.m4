...
if test "$PHP_MYSQLI" != "no"; then
  ...
  PHP_INSTALL_HEADERS([src/ext/mysqli/php_mysqli_structs.h])

  if test "$PHP_MYSQLI" = "yes" || test "$PHP_MYSQLI" = "mysqlnd"; then
    PHP_ADD_EXTENSION_DEP(mysqli, mysqlnd)
    AC_DEFINE([MYSQLI_USE_MYSQLND], 1, [Whether mysqlnd is enabled])
    PHP_INSTALL_HEADERS([src/ext/mysqli/mysqli_mysqlnd.h])
  else
    PHP_INSTALL_HEADERS([src/ext/mysqli/mysqli_libmysql.h])
  fi
fi
